...
2. **Spoken turn status.** When a turn starts/ends and when a tool runs, speak a short cue + push to the `aria-live` region: e.g. "กำลังคิด…", "กำลังอ่านไฟล์ …", "เสร็จแล้ว" / "เกิดข้อผิดพลาด". A blind user must know the agent is working without watching the screen. **The cue MUST use the same voice the user selected** — route through `speakSmart` so it inherits the active engine (system / edge / custom Miku) and the user's `voiceLang` / `speechRate` / `speechPitch` / `voiceURI` settings, exactly like read-aloud. Status, replies, and confirmations all sound like the *one* assistant the user picked — never a different/default voice.
